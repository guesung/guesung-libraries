import type { PatchCartItemsParams, PostCartItemsParams } from "@/apis";
...
export const handlers = [
	...
	http.post(`${CONFIG.baseUrl}${API_PATH.cartItems}`, async ({ request }) => {
		...
		const { productId, quantity = 1 } =
			(await request.json()) as PostCartItemsParams;
		const product = PRODUCTS_DATA.content.find(
	...
	http.delete(
		`${CONFIG.baseUrl}${API_PATH.cartItems}/:cartItemId`,
		async ({ params }) => {
			if (maybeError()) {
				await delay(MOCK_DELAY);
				return HttpResponse.json(
