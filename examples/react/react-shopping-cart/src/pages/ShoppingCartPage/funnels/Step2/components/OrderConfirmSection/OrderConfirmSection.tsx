...
export default function OrderConfirmSection() {
	...
	const selectedCartItems = cartItems.content.filter((item) =>
	...
	const totalDiscountAmount = CouponService.calculateTotalDiscountAmount(
		selectedCartItems,
		selectedCoupons,
		isFar,
	);
	...
	useEffect(() => {
		const availableCoupons = coupons.filter((coupon) =>
			new CouponService(selectedCartItems).canAdjustCoupon(coupon),
		);
		const mostDiscountCombination =
	...
	return (
        ...
        `}
			>
				<InfoIcon /> 총 주문 금액이 {FREE_DELIVERY_PRICE.toLocaleString()}원
				이상일 경우 무료 배송됩니다.
			</Text>
