...
describe("CouponService", () => {
	...
	describe("calculateDiscountPrice", () => {
		describe("FIXED5000 쿠폰", () => {
			it("5,000원의 할인 금액을 반환한다.", () => {
				expect(
					couponService.calculateDiscountPrice(COUPON_DATA[0], false),
				).toBe(5_000);
			});
		});
		...
		describe("BOGO 쿠폰", () => {
			it("2개 이상 구매한 제품이 2개 이상 있다면 더 비싼 제품을 무료로 제공한다.", () => {
				expect(
					couponService.calculateDiscountPrice(COUPON_DATA[1], false),
				).toBe(30_000);
			});
		});
		...
		describe("FREESHIPPING 쿠폰", () => {
			it("제주도 및 도서 산간 지역이 아니라면 3,000원의 할인 금액을 반환한다.", () => {
				expect(
					couponService.calculateDiscountPrice(COUPON_DATA[2], false),
				).toBe(3_000);
			});
			it("제주도 및 도서 산간 지역이라면 6,000원의 할인 금액을 반환한다.", () => {
				expect(couponService.calculateDiscountPrice(COUPON_DATA[2], true)).toBe(
					6_000,
				);
			});
		});
		...
		describe("MIRACLESALE 쿠폰", () => {
			it("30%의 할인 금액을 반환한다.", () => {
				expect(
					couponService.calculateDiscountPrice(COUPON_DATA[3], false),
				).toBe(22_500);
			});
		});
	});
	...
	describe("calculateTotalDiscountAmount", () => {
		it("총 할인 금액을 반환한다.", () => {
			expect(
				CouponService.calculateTotalDiscountAmount(
					CART_ITEMS_DATA.content,
					COUPON_DATA.slice(1, 3),
					false,
				),
			).toBe(33_000);
		});
