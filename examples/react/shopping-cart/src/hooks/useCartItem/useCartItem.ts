...
export default function useCartItem() {
	...
	const { mutate: mutatePostCartItem, status: postCartItemStatus } =
		useCartItemPostMutation();
	const { mutate: mutatePatchCartItem, status: patchCartItemStatus } =
		useCartItemPatchMutation();
	const { mutate: mutateDeleteCartItem, status: deleteCartItemStatus } =
		useCartItemDeleteMutation();

	const increaseCartItem = async (productId: number) => {
		const cartItem = cartItems.content.find(
